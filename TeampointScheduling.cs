...
namespace TeampointScheduling
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string drvlimit_sec = sAll.Get("drivelimit_sec");
            string drvlimit_ratio = sAll.Get("drivelimit_ratio");
            //Console.ReadLine();
                        ...
                        else if (capable_[i].Count() > 0)
                        {
                            ...
                            while (capable_[i].Count() > 0)
                            {
                                ...
                                */
                                ...
                                for( int j = 0; j< capable_[i].Count(); j++)
                                {
                                    //Console.Write(capable_[i][j].jobID +" " );
                                }
                                Console.WriteLine();
                                ...
                                if (initial[i].Count() == 0) //first duty
                                {
                                    ...
                                    if (capable_[i].Count() > 0) //staff has possible work 
                                    {
                                        ...
                                        for (int j = 0; j < capable_[i].Count(); j++)
                                        {
                                            ...
                                            //Calculate driving time upper limit, if there is not upper limit then set default limit as 24 hours 
                                            int drvlimit = 86400; 
                                            int a = int.Parse(drvlimit_sec); //drv limit 1
                                            int b = (int)(float.Parse(drvlimit_ratio) * capable_[i][j].duration); //drv limit 2 
                                            ...
                                            if (a != 0 && b == 0)
                                                ...
                                                drvlimit = Math.Min(a, b);


                                            //Calculate and record drvstart, workstart, workend for all the possible first job 
                                            if (find == 0)
                                                Console.WriteLine("Can't find driving time for this job.");
                                            else if (find != 0 && find <= drvlimit )
                                            {
                                                //multiple windows
                                                for (int k = 0; k < capable_[i][j].win_str.Count(); k++)
                                                {
                                                    ...
                                                    if (capable_[i][j].win_str[k] == 1)
                                                    {
                                                        ...
                                                        Console.WriteLine("firstjob case 1");
                                                    ...
                                                    if (check < staff_str)  // Jobstr-Drive < staffstr
                                                    {
                                                        ...
                                                        if (staff_str + find + capable_[i][j].duration <= capable_[i][j].win_end[k])
                                                        {
                                                            drvstr = staff_str;
                                                            workstr = staff_str + find;
                                                            Console.WriteLine("firstjob case 2");
                                                        }
                                                        else
                                                        {
                                                            Console.WriteLine("firstjob not able to do this job in this window");
                                                            Console.WriteLine("jobID:{0}, jobstr:{1}, jobend:{2}", job, capable_[i][j].win_str[k], capable_[i][j].win_end[k]);
                                                            Console.WriteLine("staffstr:{0}, driving:{1}, workstr:{2} worktime:{3},  job winend:{4}", staff_str, find, staff_str + find, capable_[i][j].duration, staff_str + find + capable_[i][j].duration);
                                                ...
                                                Console.WriteLine("Driving time for this job exceeds limit");
                                ...
                                else if (initial[i].Count() > 0) //after first duty, to assign duty based on available list
                                {
                                    if (capable_[i].Count() > 0) //staff has possible work 
                                    {
                                        ...
                                        for (int j = 0; j < capable_[i].Count(); j++)
                                        {
                                            ...
                                            //Calculate driving time upper limit
                                            //int drvlimit = Math.Min(int.Parse(drvlimit_sec), (int)(float.Parse(drvlimit_ratio) * capable_[i][j].duration));
                                            ...
                                            else if (find != 0 && find <= drvlimit)
                                            {
                                                ...
                                                for (int k = 0; k < capable_[i][j].win_str.Count(); k++)
                                                {
                                                    ...
                                                    //Case1: job start time equals 1 means workstr can be anytime && Case2: workstr is in the job window
                                                    if (capable_[i][j].win_str[k] == 1 || check < initial[i][num - 1].job_end)  // Jobstr-Drive < end of the last job
                                                    {
                                                        ...
                                                        if (initial[i][num - 1].job_end + find + capable_[i][j].duration <= capable_[i][j].win_end[k])
                                                        {
                                                            ...
                                                            Console.WriteLine("case 1 & 2 add to JobDrv: workstr after jobstr ");
                                                        } 
                                                        else
                                                        {
                                                            Console.WriteLine("case 4 reject: job cant be done in this window");
                                                            continue; //not able to do this job with this window 
                                                        }
                                                    }
                                                    //Case3: workstr in the begining of job window
                                                    else if (check >= initial[i][num - 1].job_end) //Jobstr - Duration >= staffstr
                                                    {
                                                        drvstr = check;
                                                        workstr = capable_[i][j].win_str[k];
                                                        Console.WriteLine("case 3 add to JobDrv: workstr = jobstr");
                                                    }
                                                    else
                                                    {
                                                        Console.WriteLine("\ncase 5 reject: new driving case not included");
                                                    }
                                                    ...
                                                    if (total > maxwork)
                                                        Console.WriteLine("case 6 reject: exceed max work time");
                                                    else if (workstr + capable_[i][j].duration + back > staff_end)
                                                        Console.WriteLine("case 7 reject: no time to drive back");
                                                    //else if ( maxdrive > 0 && totaldrive > maxdrive)
                                                        //Console.WriteLine("case 8 reject: exceed max drive time");
                                                    else if (workstr + capable_[i][j].duration + back <= staff_end && total <= maxwork )
                                                    //else if (workstr + capable_[i][j].duration + back <= staff_end && total <= maxwork && totaldrive <= maxdrive)
                                                    {
